...
extension Entity {
    static func placemarkEntity(for arAnchor: ARAnchor, model3D: String) -> AnchorEntity {
        let placemarkAnchor = AnchorEntity(anchor: arAnchor)
        
        let sphereIndicator = generateSphereIndicator(model3D: model3D)
    ...
    static func generateSphereIndicator(model3D : String) -> Entity {
       ...
       // modelEntity.position.y = 0 // Modifica la posizione sull'asse Y
        modelEntity.scale = [0.9, 0.9, 0.9] // Scala il modello
